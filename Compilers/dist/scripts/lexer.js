...
(function (StallCompiler) {
    var Lexer = /** @class */ (function () {
        ...
        Lexer.lex = function () {
            ...
            var matchingReg = /[a-z]+|[1-9]|(==)|(!=)|"[^"]*"|(\S)/g;
            //^ is used for the start of string/line
            var idReg = /^[a-z]+$/;
            //* for 0 or more characters
            var stringReg = /^"[a-z\s]*"$/;
            var digitReg = /0|(^[1-9]([0-9])*)$/;
            //initializes log
            _S_Logger.logIgnoreVMode("Beginning lexing.\n");
            //Gets source code.
            var sc = document.getElementById("source-code").value;
            sc = sc.trim();
                    ...
                    for (var j = 0; j < matches.length; j++) {
                        ...
                        if (keywordStr.indexOf(currentToken) > -1) {
                            for (var keywordI = 0; keywordI < _Keywords.length; keywordI++) {
                                if (currentToken === _Keywords[keywordI].value) {
                                    ...
                                    var token = StallCompiler.Token.makeNewToken(_Keywords[keywordI].type, _Keywords[keywordI].value, i + 1);
                                    _Tokens.push(token);
                            ...
                            for (var idI = 0; idI < currentToken.length; idI++) {
                                if (lexingStr) {
                                    var token = StallCompiler.Token.makeNewToken('CHARACTER', currentToken[idI], i + 1);
                                    _S_Logger.logMessage("Created '" + token.value + "' character token.");
                                }
                                else {
                                    var token = StallCompiler.Token.makeNewToken('IDENTIFIER', currentToken[idI], i + 1);
                        ...
                        //check for digit
                            ...
                            for (var digitI = 0; digitI < currentToken.length; digitI++) {
                                var token = StallCompiler.Token.makeNewToken('DIGIT', currentToken[digitI], i + 1);
                                _Tokens.push(token);
                                _S_Logger.logMessage("Created " + token.value + " token.");
                            }
                        }
                        //check for symbols
                            ...
                            for (var symbolI = 0; symbolI < _Punctuation.length; symbolI++) {
                                if (currentToken === _Punctuation[symbolI].value) {
                                    var token = StallCompiler.Token.makeNewToken(_Punctuation[symbolI].type, _Punctuation[symbolI].value, i + 1);
        ...
        Lexer.tokenizeString = function (str, line) {
            for (var i = 0; i < str.length; i++) {
                ...
                //characters, possibly
                    ...
                    var token = StallCompiler.Token.makeNewToken(CHARACTER.type, str[i], line);
                    _Tokens.push(token);
                    _S_Logger.logMessage("Created '" + token.value + "' token.");
                }
            }
